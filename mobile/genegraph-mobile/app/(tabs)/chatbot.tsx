...
import api from '../../services/api';; // نفس الـ api instance اللي بتستخدمه في login
...
export default function Chatbot() {
  ...
  // Scroll لأسفل تلقائياً لما تيجي رسالة جديدة
  useEffect(() => {
  ...
  const handleSend = async (text?: string) => {
    ...
    // أضف رسالة اليوزر على الشاشة فوراً
    ...
    try {
      ...
      // لو مفيش token أو الـ session انتهت
      ...
      const errMsg = error?.response?.data?.detail || "Failed to get response. Try again.";
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        >
          {/* لو مفيش رسائل، اعرض الـ Welcome Screen */}
          {messages.length === 0 ? (
                ...
                >
                  {msg.role === 'assistant' && (
                    <View style={styles.assistantIcon}>
                      <MaterialCommunityIcons name="robot-outline" size={16} color="#0ea5e9" />
                    </View>
                  )}
                  <View style={[
...
const styles = StyleSheet.create({
  ...
  bannerTitle: { color: '#ffffff', fontSize: 18, fontWeight: 'bold', marginBottom: 2 },
  bannerSub: { color: '#e0f2fe', fontSize: 13 },

  // Welcome Screen
  welcomeArea: { alignItems: 'center', paddingHorizontal: 24, paddingTop: 40 },
  robotIconContainer: {
    width: 70, height: 70,
  ...
  welcomeTitle: { fontSize: 22, fontWeight: '900', color: '#0f172a', marginBottom: 10 },
  welcomeText: {
    textAlign: 'center', fontSize: 14, color: '#64748b',
    lineHeight: 22, marginBottom: 30, paddingHorizontal: 10,
  },
  tryAskingText: { fontSize: 14, color: '#64748b', marginBottom: 15 },
  suggestionCard: {
  ...
  bubbleContent: {
    maxWidth: '75%', paddingVertical: 12, paddingHorizontal: 16,
    borderRadius: 18,
  },
  userBubbleContent: {
    backgroundColor: '#0ea5e9',
  ...
  assistantBubbleText: { color: '#0f172a' },
  ...
  inputContainer: {
    ...
    borderRadius: 25, minHeight: 50, maxHeight: 120,
    justifyContent: 'center',
    paddingHorizontal: 20, paddingVertical: 10, marginRight: 10,
  },
  textInput: { fontSize: 15, color: '#0f172a', maxHeight: 100 },
  sendButton: {
    width: 50, height: 50,
    backgroundColor: '#0ea5e9',
    borderRadius: 25,
