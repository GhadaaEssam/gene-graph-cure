...
import { 
  ...
  TouchableOpacity, Platform, ActivityIndicator, Alert, RefreshControl 
} from 'react-native';
...
export default function Dashboard() {
  ...
  // شاشة التحميل (Loader) لتجنب أخطاء undefined قبل وصول البيانات
  if (loading) {
    return (
  ...
  // حساب النسب المئوية للرسوم البيانية (Performance Summary)
  const total = summary?.totalAnalyses || 1;
  const resPercent = Math.round(((summary?.resistant || 0) / total) * 100);
  const senPercent = Math.round(((summary?.sensitive || 0) / total) * 100);
      ...
      >
        ...
        <View style={styles.topBanner}>
          <Text style={styles.welcomeText}>
            Welcome, Dr. {summary?.doctorName ? summary.doctorName.split(' ')[0] : 'User'}
          </Text>
        ...
        <View style={styles.contentPadding}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statCard, { backgroundColor: '#3b82f6' }]}>
              ...
              <Text style={styles.statNumber}>{summary?.totalAnalyses ?? 0}</Text>
            ...
            <View style={[styles.statCard, { backgroundColor: '#06b6d4' }]}>
              <Ionicons name="flask-outline" size={26} color="#ffffff" style={styles.statIcon} />
              <Text style={styles.statNumber}>{summary?.topPathwayCount ?? 0}</Text>
              <Text style={styles.statLabel}>{summary?.topPathway ?? 'N/A'}{"\n"}Target</Text>
            </View>
          </View>
          ...
          <View style={styles.summaryCard}>
            ...
            <View style={styles.distRow}>
              ...
              <Text style={[styles.distValue, { color: '#e11d48' }]}>{resPercent}%</Text>
            </View>
            <View style={styles.progressBarBackground}>
              <View style={[styles.progressBarFill, { width: `${resPercent}%`, backgroundColor: '#e11d48' }]} />
            </View>

            <View style={[styles.distRow, { marginTop: 15 }]}>
              <Text style={styles.distLabel}>Sensitive</Text>
              <Text style={[styles.distValue, { color: '#16a34a' }]}>{senPercent}%</Text>
            </View>
            <View style={styles.progressBarBackground}>
              <View style={[styles.progressBarFill, { width: `${senPercent}%`, backgroundColor: '#16a34a' }]} />
            </View>
          </View>
        </View>
      </ScrollView>
...
// التنسيقات (Styles) - تم تصحيح المسميات لتطابق الـ JSX
...
// ... الـ styles تفضل زي ما هي مع إضافة الـ loader
